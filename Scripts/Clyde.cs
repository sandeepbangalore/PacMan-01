...
public class Clyde : Ghosts_main {
	...
	protected override void FixedUpdate(){
		currentState = MyGameManager.GetGhostState ("Clyde");
		base.FixedUpdate ();
	}
	...
	protected override void Mover () {
		...
		if (transform.position == dest) {			
			...
			if(pacman != null){
				...
				oldDir = -dir;
				float tempDistance = (transform.position - pacmanTransform.position).magnitude;
				Vector3 scatterTransform = new Vector3 (12.5f, 0.5f, -20.5f);
				if (tempDistance >= 8.0f) {
					if (valid (right) && !oldDir.Equals (right)) {
